...
const loginHandler: NextApiHandlerWithCookie = async (req, res) => {
    ...
    try {
        ...
        const idToken = await jwt.sign({ userId: user.id }, process.env.ID_TOKEN_SECRET, {
            expiresIn: '7d',
        });

        // генерируем токен доступа
        const accessToken = await jwt.sign({ userId: user.id }, process.env.ACCESS_TOKEN_SECRET, {
            expiresIn: '1d',
        });
